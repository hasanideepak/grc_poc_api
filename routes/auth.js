...
router.post('/login', async (req, res) => {
    ...
    if (email != undefined) {
        if (password != undefined || password != '' || password != null) {
            let sql = `select a.user_id,a.email,a.org_emp_id,b.name,b.phone,c.name as org_name,c.logo,case coalesce(d.account_id,0) when 0 then 'N' else 'Y' end as is_onboard from master.users a,master.org_employees b,master.orgs c left join master.accounts d on c.org_id = d.org_id 
                       where a.username = '${email}' and a.password = '${password}' and a.org_emp_id = b.emp_id and b.org_id = c.org_id`;
